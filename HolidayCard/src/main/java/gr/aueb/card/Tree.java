...
public class Tree extends Drawable {
    ...
    /** List to store the positions of red balls */
    private List<Rectangle> redBalls = new ArrayList<>();
    ...
    private void addTreeOrnament(Graphics g, int x, int y) {
        ...
        /* Add to red balls list */
        redBalls.add(new Rectangle(x, y, 10, 10));
        /* Set color to Red. */
        g.setColor(RED);
    ...
    /**
     * Checks if a click intersects with any of the red balls.
     * 
     * @param clickPoint The point where the user clicked
     * @return true if a red ball was clicked, false otherwise
     */
    public boolean isRedBallClicked(Point clickPoint) {
        for (Rectangle ball : redBalls) {
            if (ball.contains(clickPoint)) {
                redBalls.remove(ball); // Remove clicked ball
                return true;
            }
        }
        return false;
